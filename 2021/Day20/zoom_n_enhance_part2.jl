...
function solution(path::String)
    ...
    zoomed_screen = expand_initial_screen(screen)
    
    enhance_count = 0
    for _ in 1:50
        zoomed_screen = zoom_screen(zoomed_screen, pattern, enhance_count)
        enhance_count += 1
        # print(size(zoomed_screen), "\n")
    end
...
# everystep the screen expands by 1 on every direction, but only the non-buffered content are carried to the 
# expanded screen
